...
const productSchema = z.object({
  ...
  specs: z
    ...
    .min(3),
  gradeTable: z
    .array(z.object({ parameter: z.string(), range: z.string() }))
    .optional(),
  ...
  illustration: z.object({
    id: z.string(),
    alt: z.string().startsWith("Illustration of"),
  }),
  seo: z.object({
...
function shelfLifeFrom(
  specs: { label: string; value: string }[],
  fallback = "12–24 months",
): string {
...
function merge(g: Generated): Product {
  ...
  if (!o)
    throw new Error(
      `content: missing override for product "${g.id}" (content/overrides/products.ts)`,
    );
  const profile = g.profile ?? o.profile;
  if (!profile)
    throw new Error(
      `content: product "${g.id}" has neither a docx profile nor an authored one`,
    );
  const specs = g.specs.length ? g.specs : (o.specs ?? []);
  const originRegions = o.originRegions?.length
    ? o.originRegions
    : g.originRegions;
  return {
    id: g.id,
    ...
    synonyms: g.synonyms,
    profile: {
      source: profile.source as "docx" | "authored",
      science: profile.science,
      whyIndian: profile.whyIndian,
