...
export const metadata: Metadata = createPageMetadata({
  ...
  description:
    "Build a sample kit from 27 Indian-origin ingredients, tell us your market, and get a quote against today's crop by email and WhatsApp. FOB and CIF.",
  path: "/inquiry",
});

const BUYER_TYPES = (Object.keys(buyerTypeLabel) as BuyerTypeId[]).map(
  (id) => ({ id, label: buyerTypeLabel[id] }),
);

export default async function InquiryPage({
  searchParams,
}: {
  searchParams: Promise<{ product?: string; kit?: string }>;
}) {
  const sp = await searchParams;
  const preselect = [sp.product, ...(sp.kit?.split(",") ?? [])].filter(
    (s): s is string => !!s,
  );
  return (
    <>
      <section
        data-theme="dark"
        className="grain bg-forest-950 pt-32 pb-14 text-cream-50 md:pt-40 md:pb-20"
      >
        <div className="container-x">
          ...
          <p className="mt-6 max-w-2xl text-lead text-cream-100/85">
            Pick products, add the spec brief and documents you need, tell us
            your market. We quote against today&apos;s crop — by email and
            WhatsApp. {cta.samples}
          </p>
        </div>
      </section>
      ...
      >
        <div className="container-x">
          <KitBuilder
            products={toKitProducts(products)}
            buyerTypes={BUYER_TYPES}
            clusters={orderedClusters.map((c) => ({
              id: c.id,
              name: c.shortName,
            }))}
            preselect={preselect}
        ...
        <div className="container-x grid gap-12 lg:grid-cols-12">
          <div className="lg:col-span-5">
            ...
            <p className="mt-4 text-body text-ink-700">
              No kit needed. Documents, availability, whether we can do a spec —
            ...
            <ul className="mt-10 space-y-4 text-body">
              <li className="flex gap-3">
                ...
                <a
                  ...
                  data-umami-event-placement="inquiry_contact"
                >
