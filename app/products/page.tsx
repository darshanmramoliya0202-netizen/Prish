    ...
    <SiteShell>
      <section className="mx-auto max-w-7xl px-4 pb-12 pt-10 sm:px-6 sm:pb-16 sm:pt-16 lg:px-10 lg:pt-24">
        <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.4em] text-saffron">Products</p>
            <h1 className="mt-5 max-w-4xl font-sans text-3xl font-bold text-[#0f172a] sm:text-5xl lg:text-6xl">Ingredients organized by formulation logic, not brochure blocks.</h1>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {applicationMatrix.map((item) => (
              <div key={item.sector} className="rounded-2xl border border-slate-200 bg-white p-6">
                ...
                <p className="mt-4 text-sm leading-7 text-slate-600">{item.requirement}</p>
                <div className="mt-5 flex flex-wrap gap-2">
                  {item.solutions.map((solution) => (
                    <span key={solution} className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs uppercase tracking-[0.18em] text-slate-600">
                      {solution}
                    </span>
                  ))}
                </div>
            ...
            <article key={category.slug} className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
              <div className="grid gap-8 lg:grid-cols-[0.78fr_1.22fr]">
                <div>
                  ...
                  <h2 className="mt-4 font-sans text-3xl font-bold text-[#0f172a]">{category.title}</h2>
                  <p className="mt-4 text-sm leading-7 text-slate-600">{category.overview}</p>
                  <div className="mt-6 rounded-xl border border-slate-300 bg-slate-50 p-5 text-sm leading-7 text-slate-600">
                  ...
                  <div className="grid gap-4 xl:grid-cols-3">
                    {category.products.map((product) => (
                      <div key={product.name} className="rounded-xl border border-slate-200 bg-white p-5">
                        <h3 className="font-sans text-xl font-bold text-[#0f172a]">{product.name}</h3>
                        <p className="mt-3 text-sm leading-7 text-slate-600">{product.profile}</p>
                        <div className="mt-5 flex flex-wrap gap-2">
                          {product.useCases.map((useCase) => (
                            <span key={useCase} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] uppercase tracking-[0.18em] text-slate-600">
                              {useCase}
                            </span>
                          ))}
                        </div>
