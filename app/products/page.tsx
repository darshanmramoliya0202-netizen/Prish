...
export const metadata: Metadata = createPageMetadata({
  ...
  description:
    "Fruit, vegetable and herbal powders, dehydrated onion and garlic, raw whole spices, moringa and basmati rice. One family per screen, every product with its origin belt and paperwork.",
  path: "/products",
});

/** One family per screen (owner note). Scroll-snap on touch; the pinned version arrives with motion. */
export default function ProductsPage() {
  const entries = toSearchEntries(
    products,
    productPath,
    (p) => clusterOf(p).shortName,
  );
  return (
    <>
      <JsonLd
      ...
      <section
        data-theme="dark"
        className="bg-forest-950 pt-32 pb-12 text-cream-50"
      >
        <div className="container-x flex flex-col items-start gap-6 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-xl text-lead opacity-85">
              One family per screen. Tap any bowl for its origin belt, typical
              specs and the documents your market asks for.
            </p>
          </div>
              ...
              <div className="container-x grid gap-10 lg:grid-cols-12 lg:items-center">
                <div className="lg:col-span-4">
                  <Accent
                    accent={c.accent}
                    size="md"
                    className={dark ? "text-gold-400" : "text-gold-700"}
                  />
                ...
                <ul
                  className={`grid gap-2 lg:col-span-8 ${list.length <= 2 ? "grid-cols-2 max-w-md" : list.length <= 5 ? "grid-cols-3 sm:grid-cols-5" : "grid-cols-3 sm:grid-cols-4"}`}
                  role="list"
                >
                  {list.map((p) => (
                    <li key={p.id}>
                      ...
                      >
                        ...
                        {p.desiName ? (
                          <span className="eyebrow mt-1 block opacity-60">
                            {p.desiName.roman}
                          </span>
                        ) : null}
                      </Link>
      ...
      >
        <div className="container-x text-center">
          ...
          <p className="mx-auto mt-4 max-w-xl text-lead text-ink-700">
            We add lines when buyers ask and when we can source them with a
            straight face. Tell us what you need.
          </p>
