...
import { getClusterBySlug, orderedClusters, productsByCluster, clusterPath, buyerTypeLabel, calendarFor, flagText, cta } from "@/content";
...
export async function generateMetadata({ params }: { params: Promise<{ cluster: string }> }): Promise<Metadata> {
  ...
  if (!c) return {};
  return createPageMetadata({ title: `${c.name} from India`, description: `${c.promise} ${c.description}`.slice(0, 200), path: clusterPath(c) });
}
...
export default async function ClusterPage({ params }: { params: Promise<{ cluster: string }> }) {
  ...
  const buyers = Array.from(new Set(list.flatMap((p) => p.buyerTypes))) as BuyerTypeId[];
  ...
  return (
    <>
      ...
      <section
        ...
        style={{ ["--world" as string]: c.colourWorld.primary, ["--world-2" as string]: c.colourWorld.secondary, ["--world-ink" as string]: dark ? "#fbf8f1" : "#14110c", background: `linear-gradient(160deg, ${c.colourWorld.primary}, ${c.colourWorld.secondary})` }}
      >
            ...
            <div className="lg:col-span-4">
              ...
              <ul className="mt-3 flex flex-wrap gap-2">
                {buyers.slice(0, 6).map((b) => (
                  <li key={b}>
                    <Pill className="border-current/40">{buyerTypeLabel[b]}</Pill>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section data-theme="light" className="bg-cream-100 py-16 text-ink-900 md:py-24">
        <div className="container-x">
          ...
          <h2 className="mt-3 text-display-lg">What your market will ask for.</h2>
          {flags.length ? (
            <ul className="mt-6 flex flex-wrap gap-2">
              {flags.map((f) => (
                <li key={f} className="max-w-md rounded-lg border border-ink-900/10 bg-cream-50 p-4 text-small text-ink-700">
                  {flagText[f]}
                </li>
              ))}
            </ul>
      ...
      <section data-theme="light" className="bg-cream-50 py-16 text-ink-900 md:py-24">
        <div className="container-x grid gap-10 lg:grid-cols-12">
          <div className="lg:col-span-4">
            ...
            <p className="mt-4 text-body text-ink-700">New crop means fresher colour and, often, better pricing. Indicative windows — ask us for the current season.</p>
            <ButtonLink href="/crop-calendar" variant="secondary" className="mt-6">
          ...
          <ul className="space-y-5 lg:col-span-8">
            {list.map((p) => {
              ...
              if (!row) return null;
              return (
                <li key={p.id} className="grid items-center gap-3 sm:grid-cols-12">
                  <span className="font-semibold sm:col-span-3">{p.shortName}</span>
      ...
      <section data-theme="dark" className="bg-forest-950 py-16 text-cream-50 md:py-24">
        <div className="container-x flex flex-col items-start gap-6 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-display-lg">Source {c.shortName.toLowerCase()} from us.</h2>
            <p className="mt-3 max-w-xl text-lead opacity-85">Add what you need to a sample kit and we quote against today&apos;s crop.</p>
          </div>
