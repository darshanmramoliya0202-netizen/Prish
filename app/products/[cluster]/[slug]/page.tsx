...
import { allProductParams, getProductBySlug, getClusterBySlug, productsByCluster, productPath, clusterPath, buyerTypeLabel, calendarFor, flagText, cta, hsDisclaimer } from "@/content";
...
export async function generateMetadata({ params }: { params: Promise<{ cluster: string; slug: string }> }): Promise<Metadata> {
  ...
  return createPageMetadata({ title: p.seo.title, description: p.seo.description, path: productPath(p), image: `${productPath(p)}/opengraph-image` });
}

export default async function ProductPage({ params }: { params: Promise<{ cluster: string; slug: string }> }) {
  const { cluster, slug } = await params;
  const p = getProductBySlug(slug);
  const c = getClusterBySlug(cluster);
  if (!p || !c || p.cluster !== c.id) notFound();
  const siblings = productsByCluster(c.id).filter((x) => x.id !== p.id).slice(0, 4);
    ...
    <>
        ...
        data={[
          {
            ...
            category: c.name,
            countryOfOrigin: "IN",
            additionalProperty: [{ "@type": "PropertyValue", name: "HS code (6-digit)", value: p.hs.hs6 }],
          ...
          },
          {
            ...
            "@type": "BreadcrumbList",
            itemListElement: [
              { "@type": "ListItem", position: 1, name: "Home", item: resolveUrl("/") },
              { "@type": "ListItem", position: 2, name: "Products", item: resolveUrl("/products") },
              { "@type": "ListItem", position: 3, name: c.name, item: resolveUrl(clusterPath(c)) },
              { "@type": "ListItem", position: 4, name: p.name, item: resolveUrl(productPath(p)) },
            ],
          },
        ]}
      />
        ...
        <div className="container-x grid gap-10 lg:grid-cols-12 lg:items-center">
          <div className="lg:col-span-6">
            <nav aria-label="Breadcrumb" className="text-small opacity-75">
              ...
              /{" "}
              <Link href={clusterPath(c)} className="underline-offset-4 hover:underline">
                {c.name}
              </Link>
            </nav>
            {p.desiName ? (
              <p className="mt-6">
                <span lang={p.desiName.lang} className="font-deva text-display-lg leading-none">
                  {p.desiName.text}
                </span>
                <span className="eyebrow ml-3 opacity-75">{p.desiName.roman}</span>
              </p>
            ) : null}
            <h1 className="mt-3 text-display-xl">{p.name}</h1>
            <p className="mt-5 max-w-xl text-lead opacity-90">{p.profile.whyIndian}</p>
            <ul className="mt-6 flex flex-wrap gap-2">
              ...
              <li>
                <Pill className="border-current/40 tabular">HS {p.hs.hs6.slice(0, 4)}.{p.hs.hs6.slice(4)}</Pill>
              </li>
              {p.hs.verified && p.hs.itcHs ? (
                <li>
                  <Pill className="border-current/40 tabular">ITC-HS {p.hs.itcHs}</Pill>
                </li>
              ) : null}
            </ul>
            <div className="mt-8 flex flex-wrap gap-3">
              <AddToKitButton productId={p.id} size="lg" className="!border-current/60" />
              <a
          ...
          <div className="lg:col-span-6">
            <ProductBowl product={p} decorative={false} className="mx-auto w-full max-w-lg drop-shadow-2xl" data-hero-bowl />
          </div>
        </div>
      </section>
      ...
      <section data-theme="light" className="bg-cream-50 py-16 text-ink-900 md:py-24">
        <div className="container-x grid gap-10 lg:grid-cols-12 lg:items-center">
          ...
          <div className="lg:col-span-7">
            <OriginMap origins={p.originRegions} accent={p.colourWorld.primary} className="mx-auto w-full max-w-md text-forest-900" />
          </div>
        </div>
      </section>
      ...
      <section data-theme="light" className="bg-cream-100 py-16 text-ink-900 md:py-24">
        <div className="container-x grid gap-10 md:grid-cols-3">
          ...
          {p.profile.source === "authored" ? <p className="text-small text-ink-500 md:col-span-3">Profile summarised from published ingredient science; lot-specific data on the COA.</p> : null}
        </div>
      </section>
      ...
      <section data-theme="light" className="bg-cream-50 py-16 text-ink-900 md:py-24">
        <div className="container-x grid gap-12 lg:grid-cols-12">
          <div className="lg:col-span-6">
            ...
            <h2 className="mt-3 text-display-lg">The numbers we quote against.</h2>
            <div className="mt-8">
              <SpecTable specs={p.specs} />
            </div>
          </div>
          <div className="lg:col-span-6">
            {p.gradeTable ? <GradeTable rows={p.gradeTable} title={`${p.shortName} grades`} /> : null}
            {p.variants ? (
        ...
        <div className="container-x">
          ...
          {p.flags.length ? (
            <ul className="mt-6 grid gap-3 md:grid-cols-2">
              {p.flags.map((f) => (
                <li key={f} className="rounded-lg border border-ink-900/10 bg-cream-50 p-4 text-small text-ink-700">
                  {flagText[f]}
                </li>
              ))}
            </ul>
            ...
            <ul className="mt-6 space-y-2 text-body text-ink-700">
              {p.packaging.map((x) => (
                <li key={x} className="flex gap-3">
                  <span className="mt-2.5 size-1.5 shrink-0 rounded-full bg-gold-500" aria-hidden /> {x}
                </li>
              ))}
            </ul>
            <p className="mt-6 text-body">
              Shelf life: <strong>{p.shelfLife}</strong> in cool, dry, hygienic storage. FCL and LCL.
            </p>
          </div>
          <div className="lg:col-span-7">
      ...
      {/* 7 — get it */}
      <section data-theme="dark" className="bg-forest-950 py-16 text-cream-50 md:py-24">
        <div className="container-x grid gap-8 lg:grid-cols-12 lg:items-center">
          <div className="lg:col-span-7">
            <h2 className="text-display-lg">Get {p.shortName.toLowerCase()} moving.</h2>
            <p className="mt-3 max-w-xl text-lead opacity-85">Add it to a sample kit, download the spec sheet, or ask for today&apos;s price. {cta.samples}</p>
          </div>
          <div className="flex flex-wrap gap-3 lg:col-span-5 lg:justify-end">
            <AddToKitButton productId={p.id} size="lg" />
            <a href={specPdf} className="inline-flex h-13 items-center gap-2 rounded-full border border-cream-50/40 px-7 text-lead font-semibold hover:border-cream-50" download data-umami-event="spec_sheet_download" data-umami-event-product={p.slug}>
