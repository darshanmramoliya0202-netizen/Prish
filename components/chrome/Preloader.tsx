...
export function Preloader() {
  ...
  const [line] = useState(
    () => loaderLines[Math.floor(Math.random() * loaderLines.length)],
  );
  ...
  return (
    <div
      ref={el}
      aria-hidden
      className="fixed inset-0 z-[90] grid place-items-center bg-forest-950 text-cream-50 grain"
    >
      <div className="text-center">
        <Seal decorative data-seal className="mx-auto size-24" />
        <p
          data-line
          className="mt-6 font-display text-display-md italic text-gold-300"
        >
          {line}
        </p>
      </div>
