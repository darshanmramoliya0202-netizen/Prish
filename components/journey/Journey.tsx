...
export function Journey() {
  ...
  // Deferred by one frame: during hydration `reduced` is still the server value, and the
  // client re-render (which may swap to the storyboard) must win before GSAP pins anything.
  useEffect(() => {
    ...
    const raf = requestAnimationFrame(() => {
      ctx = gsap.context(() => {
        ...
        const tl = gsap.timeline({
          ...
          scrollTrigger: {
            ...
            onUpdate: (self) => {
              ...
              dots.forEach((d, k) =>
                d.setAttribute("data-on", k === i ? "true" : "false"),
              );
            },
          },
        });
        ...
        if (sun)
          tl.fromTo(
            sun,
            { rotate: -20, transformOrigin: "center" },
            { rotate: 20 },
            2,
          );
        ...
        tl.to(
          arcs,
          { strokeDashoffset: 0, duration: 0.6, stagger: 0.05 },
          last + 0.05,
        );
        tl.to(
          ends,
          { scale: 1, duration: 0.15, stagger: 0.05, ease: "back.out(2)" },
          last + 0.45,
        );
        if (pulse)
          tl.fromTo(
  ...
  return (
    <section
      ...
      data-theme="dark"
      data-journey
      className="relative h-dvh overflow-hidden bg-forest-950 text-cream-50"
    >
      {/* scenes */}
            ...
            >
              {Art ? (
                ...
                <div className="grid h-full w-full place-items-center bg-forest-950 p-6 pb-40 md:p-16 md:pb-16">
                  <WorldMap
                    className="h-auto w-full max-w-5xl text-cream-50"
                    highlight={region}
                  />
                </div>
              )}
              ...
              <div
                aria-hidden
                className="absolute inset-x-0 bottom-0 h-2/3 bg-gradient-to-t from-forest-950/90 via-forest-950/40 to-transparent md:h-1/2"
              />
            </div>
          );
        })}
      ...
      >
        {journey.scenes.map((s) => (
          <li
            key={s.id}
            data-scene-dot
            className="size-2 rounded-full bg-cream-50/30 transition-colors data-[on=true]:bg-gold-400"
          />
        ))}
      </ol>
