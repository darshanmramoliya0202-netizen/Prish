...
export function JourneyStoryboard() {
  return (
    <section data-theme="dark" data-journey className="relative grain bg-forest-950 py-section text-cream-50">
      <div className="container-x">
        <SectionHeading eyebrow="Traceability" title={home.journeyTitle} sub={home.journeySub} />
        <ol className="mt-16 space-y-20">
          {journey.scenes.map((s) => {
            const Art = s.id === "world" ? null : ART[s.id as keyof typeof ART];
            return (
              <li key={s.id} className="grid items-center gap-8 lg:grid-cols-12" data-scene={s.id}>
                <div className="lg:col-span-4">
                  <p className="font-display text-display-xl text-gold-400 leading-none">{s.index}</p>
                  <h3 className="mt-4 text-display-md">{s.title}</h3>
                  <p className="mt-4 text-body text-cream-100/85">{s.caption}</p>
                  {s.accent ? (
                    <p className="mt-6">
                ...
                <div className="overflow-hidden rounded-xl shadow-deep lg:col-span-8">
                  {Art ? <Art className="block w-full" /> : <WorldMap className="block w-full bg-forest-900 p-6 text-cream-50" />}
                </div>
              </li>
            );
          })}
