...
const frame = (p: P, label: string) => ({ viewBox: "0 0 1200 700", preserveAspectRatio: "xMidYMid slice", role: "img", "aria-label": label, ...p });

/* rows of a ploughed field, curving toward a vanishing point */
function FieldRows({ y0 = 420, n = 9, color = SOIL2, opacity = 0.9 }: { y0?: number; n?: number; color?: string; opacity?: number }) {
  const rows = Array.from({ length: n }, (_, i) => {
    const t = i / (n - 1);
    const y = y0 + t * t * 280;
    const bulge = 40 + t * 140;
    return <path key={i} d={`M -50 ${y} Q 600 ${y - bulge} 1250 ${y}`} stroke={color} strokeOpacity={opacity} strokeWidth={2 + t * 6} fill="none" />;
  });
  return <g>{rows}</g>;
}

function Tree({ x, y, s = 1, color = F950 }: { x: number; y: number; s?: number; color?: string }) {
  return (
...
export function SceneSoil(p: P) {
  return (
    <svg {...frame(p, "Illustration of a ploughed Saurashtra field at dawn with a neem tree")}>
      <defs>
      ...
      <g data-layer="fg">
        ...
        <g transform="translate(600 560)">
          ...
          <ellipse cx="0" cy="0" rx="96" ry="40" fill={GOLD} transform="rotate(-12)" />
          <path d="M-60 -6 Q 0 -30 62 -2" stroke={GOLD3} strokeWidth="5" fill="none" strokeLinecap="round" transform="rotate(-12)" />
        </g>
      </g>
    </svg>
  );
...
export function SceneHarvest(p: P) {
  return (
    <svg {...frame(p, "Illustration of harvest: cupped hands lifting seeds above a filled sack")}>
      <defs>
      ...
      <g data-layer="mid">
        ...
        <g transform="translate(880 470)">
          <path d="M-110 0 Q -120 190 -80 210 L 80 210 Q 120 190 110 0 Z" fill={CREAM} />
          <path d="M-110 0 Q 0 40 110 0" fill="none" stroke={SOIL2} strokeWidth="6" />
          <ellipse cx="0" cy="-6" rx="96" ry="30" fill={GOLD} />
          <path d="M-70 60 H 70 M-64 100 H 64" stroke={SOIL2} strokeWidth="3" strokeDasharray="10 8" />
        </g>
      </g>
      <g data-layer="fg">
        {/* cupped hands, line art, cream */}
        <g transform="translate(430 470)" fill="none" stroke={CREAM} strokeWidth="7" strokeLinecap="round" strokeLinejoin="round">
          ...
          <path d="M-140 30 C -100 10, -60 10, -20 30 M20 30 C 60 10, 100 10, 140 30" strokeWidth="4" opacity="0.6" />
        </g>
        {/* seeds in the hands */}
        <g fill={GOLD}>
          {Array.from({ length: 34 }, (_, i) => {
            ...
            const r = 30 + (i % 5) * 18;
            return <ellipse key={i} cx={430 + Math.cos(a) * r * 2.2} cy={440 + Math.sin(a) * r * 0.55} rx="9" ry="4" transform={`rotate(${(i * 37) % 180} ${430 + Math.cos(a) * r * 2.2} ${440 + Math.sin(a) * r * 0.55})`} />;
          })}
        </g>
      </g>
    </svg>
...
export function SceneSun(p: P) {
  ...
  return (
    <svg {...frame(p, "Illustration of a large sun over rows of drying trays of coloured powders")}>
      <defs>
      ...
      <g data-layer="bg">
        ...
        <g transform="translate(600 250)" data-sun>
          {rays.map((a) => (
            <line key={a} x1="0" y1="-170" x2="0" y2="-215" stroke={F900} strokeWidth="5" strokeLinecap="round" transform={`rotate(${a})`} opacity="0.55" />
          ))}
          <circle r="150" fill={F900} />
          <circle r="120" fill={GOLD3} />
          <text y="14" textAnchor="middle" fontFamily="var(--font-display)" fontSize="64" fill={F900} fontWeight="600">
            280+
          </text>
          <text y="48" textAnchor="middle" fontFamily="var(--font-sans)" fontSize="18" fill={F900} letterSpacing="4">
            SUNSHINE DAYS
          </text>
        </g>
      </g>
      <g data-layer="mid">
        ...
        {[
          ...
          <g key={i} transform={`translate(${x} 520)`}>
            ...
            <path d="M-104 12 L 104 12 L 128 68 L -128 68 Z" fill={c as string} />
          </g>
        ))}
      </g>
      <g data-layer="fg">
...
export function SceneMill(p: P) {
  return (
    <svg {...frame(p, "Illustration of a powder mound beside a fibre drum, an export bag and a bulk box")}>
      <g data-layer="bg">
      ...
      <g data-layer="mid">
        {/* mound */}
        <path d="M120 470 C 200 380, 300 330, 360 330 C 420 330, 520 380, 600 470 Z" fill="#e0a106" />
        <path d="M280 372 C 320 350, 380 344, 430 356" stroke={GOLD3} strokeWidth="8" fill="none" strokeLinecap="round" opacity="0.7" />
        {/* fibre drum */}
        <g transform="translate(760 470)">
          ...
          <path d="M-80 -150 H 80 M-80 -60 H 80" stroke={F900} strokeWidth="5" opacity="0.5" />
        </g>
        {/* HDPE bag */}
        <g transform="translate(960 470)">
          <path d="M-90 0 L -84 -180 Q 0 -230 84 -180 L 90 0 Z" fill={CREAM} stroke={F900} strokeWidth="4" />
          <rect x="-46" y="-130" width="92" height="60" rx="6" fill={F900} />
          <text x="0" y="-92" textAnchor="middle" fontFamily="var(--font-sans)" fontSize="18" fill={CREAM} letterSpacing="3">
            LOT
          </text>
        </g>
        {/* bulk box */}
...
export function SceneCoast(p: P) {
  const boxes = ["#e0a106", "#a5133f", "#145a3a", "#c4341f", "#3f1a36", "#d4a24c"];
  return (
    <svg {...frame(p, "Illustration of a container ship leaving a coast with port cranes at dusk")}>
      <defs>
        ...
        <g transform="translate(720 470)" data-ship>
          ...
          {boxes.map((c, i) => (
            <g key={i}>
              ...
              <rect x={-240 + i * 60} y="-92" width="54" height="44" fill={boxes[(i + 3) % 6]} opacity="0.9" />
            </g>
          ))}
        </g>
      </g>
      <g data-layer="fg">
        {[0, 1, 2].map((i) => (
          <path key={i} d={`M-60 ${560 + i * 40} Q 200 ${540 + i * 40} 460 ${560 + i * 40} T 980 ${560 + i * 40} T 1500 ${560 + i * 40}`} fill="none" stroke={CREAM} strokeOpacity={0.25 - i * 0.06} strokeWidth="3" />
        ))}
      </g>
    </svg>
  );
