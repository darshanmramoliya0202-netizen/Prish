...
export function AuditUs() {
  const held = certificates.filter(
    (c) => c.status === "held" && c.showOn.includes("quality"),
  );
      ...
      <div className="container-x grid gap-12 lg:grid-cols-12">
        ...
        >
          {held.map((c) => (
            <li
              key={c.id}
              className="rounded-lg border border-ink-900/10 bg-cream-50 p-5 shadow-paper"
              data-reveal
            >
              ...
              <p className="mt-4">
                {c.number ? (
                  <Pill tone="ok">On file · {c.number}</Pill>
                ) : (
                  <Pill tone="muted">Held · number on request</Pill>
                )}
              </p>
            </li>
          ))}
          {pending.map((c) => (
            <li
              key={c.id}
              className="rounded-lg border border-dashed border-ink-900/20 p-5"
              data-reveal
            >
              <p className="font-display text-display-md leading-tight">
                {c.name}
        ...
        <p className="text-small text-ink-500 lg:col-span-12">
          Every consignment ships with a lot-specific Certificate of Analysis.
