...
export function Portrait({
  ...
  return (
    <figure
      className={`relative aspect-[4/5] w-full max-w-md overflow-hidden rounded-xl bg-forest-900 shadow-deep ${className}`}
    >
      ...
      {person.photo ? (
        ...
        <div className="grid h-full place-items-center text-cream-50">
          <span className="font-display text-[8rem] leading-none text-gold-400">
            {initials}
          </span>
        </div>
      )}
      ...
      <figcaption className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-forest-950/90 to-transparent p-5 text-cream-50">
        ...
        <span className="eyebrow mt-1 block opacity-80">
          {person.role} · Rajkot
        </span>
      </figcaption>
    </figure>
