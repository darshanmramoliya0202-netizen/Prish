...
async function main() {
  ...
  for (const p of products) {
    ...
    if (manifest[p.slug] === hash && existsSync(svgPath) && existsSync(pngPath)) continue;
    writeFileSync(svgPath, svg);
    await sharp(Buffer.from(svg), { density: 200 }).resize(1000, 1000).png({ compressionLevel: 9 }).toFile(pngPath);
    manifest[p.slug] = hash;
    rendered++;
  }
  writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));
  console.log(`render:bowls — ${rendered} rendered, ${products.length - rendered} unchanged`);
}
