...
import { products, clusters, regions, certificates, flagText, buyerTypeLabel, getCluster, productsByCluster, productPath } from "../content";
...
const SITE = (process.env.NEXT_PUBLIC_SITE_URL ?? "https://www.prishoverseas.com").replace(/\/+$/, "");
...
async function qr(url: string) {
  const buf = await QRCode.toBuffer(url, { width: 320, margin: 1, color: { dark: "#0b3d2e", light: "#fbf8f1" } });
...
async function main() {
  ...
  for (const p of products) {
    ...
    const hash = createHash("sha1").update(JSON.stringify(p)).update(templateHash).digest("hex").slice(0, 12);
  ...
  const catHash = createHash("sha1").update(JSON.stringify(products)).update(JSON.stringify(certificates)).update(templateHash).digest("hex").slice(0, 12);
  const catOut = join(OUT, "prish-overseas-catalogue.pdf");
  if (manifest.catalogue !== catHash || !existsSync(catOut)) {
    const coverBowls = clusters.slice(0, 4).map((c) => png(join(ILLUS, `${products.find((p) => p.id === c.heroProductId)!.slug}.png`)));
    await renderToFile(
      <CatalogueDocument
        clusters={clusters}
  ...
  console.log(`build:pdfs — ${rendered} rendered, ${products.length + 1 - rendered} unchanged`);
}
