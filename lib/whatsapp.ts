...
export function waLink(opts: WaOpts = {}): string {
  ...
  const who = opts.name ? ` this is ${opts.name}${opts.company ? ` from ${opts.company}` : ""}${opts.country ? ` (${opts.country})` : ""}.` : "";
  const what = opts.products?.length ? ` Interested in: ${opts.products.join(", ")}.` : "";
