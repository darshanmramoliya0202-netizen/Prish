...
export function CatalogueDocument(c: CatalogueProps) {
  ...
  return (
    <Document
      title={`${site.company} — Product catalogue ${c.date.slice(0, 4)}`}
      author={site.company}
    ...
    >
      ...
      <Page
        ...
        style={[
          s.page,
          { backgroundColor: C.forestDeep, color: C.cream, paddingBottom: 0 },
        ]}
      >
        <View
          style={{
        ...
        >
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            <Image
              src={c.sealCreamPng}
              style={{ width: 54, height: 54, marginRight: 14 }}
            />
            <View>
              <Text style={{ fontSize: 11, letterSpacing: 3, fontWeight: 700 }}>
                PRISH OVERSEAS
              </Text>
          ...
          <View>
            <Text style={{ fontSize: 9, opacity: 0.85 }}>
              {c.clusters.length} families ·{" "}
              {c.clusters.reduce(
                (n, cl) => n + c.productsByCluster(cl.id).length,
                0,
              )}{" "}
              products · United States · European Union · GCC · Southeast Asia
            </Text>
      ...
      <Page size="A4" style={s.page}>
        <Band
          sealPng={c.sealPng}
          title="ABOUT"
          sub="Farm-rooted in Saurashtra"
        />
        <View style={s.body}>
          <Text style={s.h1}>Indian origin, documented for your market.</Text>
          <Text style={[s.p, { marginTop: 10 }]}>
            India has long been a trusted source of high-quality agricultural
            and botanical ingredients. {site.company} offers fruit, vegetable
            and herbal powders, dehydrated onion and garlic, whole spices,
            moringa and basmati rice — responsibly sourced from selected Indian
            farms and processed with controlled drying and milling so nutrient
            content, functional performance and quality stay consistent. Purity,
            traceability and export compliance are the point.
          </Text>
          ...
          {[
            "Multiple agro-climatic zones supporting diverse botanicals",
            `${site.sunshineDays} annual sunshine days in the major growing regions`,
            "Traditional farming expertise combined with modern processing",
            "Cost-effective production with year-round availability",
            "Structured sourcing for traceable, export-oriented quality",
          ].map((x) => (
          ...
          <Text style={[s.h2, { marginTop: 18 }]}>
            Families in this catalogue
          </Text>
          {c.clusters.map((cl) => (
            <View
              key={cl.id}
              style={[s.tr, { justifyContent: "space-between" }]}
            >
              <Text style={{ fontWeight: 600, fontSize: 9.5 }}>{cl.name}</Text>
              <Text style={[s.small, { width: "55%", textAlign: "right" }]}>
                {c
                  .productsByCluster(cl.id)
                  .map((p) => p.shortName)
                  .join(" · ")}
              </Text>
            </View>
        ...
        <Footer
          left={`${site.company} · Rajkot, Gujarat, India`}
          right={`Catalogue v${c.version}`}
        />
      </Page>
        ...
        <View key={cl.id}>
          <Page
            ...
            style={[
              s.page,
              {
                backgroundColor: cl.colourWorld.primary,
                color: cl.colourWorld.ink === "light" ? C.cream : C.ink,
                paddingBottom: 0,
              },
            ]}
          >
            ...
            >
              ...
              <Text style={{ fontSize: 8.5, marginTop: 18, opacity: 0.8 }}>
                {c
                  .productsByCluster(cl.id)
                  .map((p) => p.name)
                  .join(" · ")}
          ...
          <View style={[s.row, { marginTop: 16 }]}>
            ...
            <View style={{ width: "50%", paddingLeft: 12 }}>
              ...
              <Text style={[s.p, { marginTop: 6 }]}>
                Shelf life 12–24 months in cool, dry, hygienic storage. FCL and
                LCL.
              </Text>
            </View>
          </View>
      ...
      <Page
        ...
        style={[
          s.page,
          { backgroundColor: C.forest, color: C.cream, paddingBottom: 0 },
        ]}
      >
        <View
          style={{
        ...
        >
          <View>
            ...
            >
              Pick products, tell us your market, and we quote against today’s
              crop.
            </Text>
            <Text
              style={{
            ...
            >
              Samples available — ask us. Incoterms{" "}
              {site.incoterms.join(" and ")}. Lot-specific Certificate of
              Analysis with every consignment. Third-party testing on request.
            </Text>
          </View>
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            <Image
            ...
            <View>
              ...
              <Text style={{ fontSize: 9.5, marginTop: 6 }}>
                WhatsApp / phone: {site.phones.join(" · ")}
              </Text>
              <Text style={{ fontSize: 9.5 }}>
                {site.email} · prishoverseas.com/inquiry
              </Text>
            </View>
          </View>
        </View>
