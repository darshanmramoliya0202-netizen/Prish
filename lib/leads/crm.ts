...
export function crmReachable(): boolean {
  try {
    return (
      crmEnabled() &&
      existsSync(env("CRM_PYTHON")) &&
      existsSync(join(env("CRM_DIR"), "src", "import_leads.py"))
    );
  } catch {
    return false;
  }
...
export function leadToCsv(l: StoredLead): string {
  ...
  const products =
    l.lead.kind === "sample_kit"
      ? l.lead.items.map((i) => getProduct(i.productId)?.name ?? i.productId)
      : [l.lead.productInterest].filter(Boolean);
  const region = l.lead.region ? regionById[l.lead.region]?.short : "";
  const parts: string[] = [
    `Website ${l.ref} ${l.receivedAt.slice(0, 10)}`,
    l.lead.kind === "sample_kit" ? "sample kit" : "quick inquiry",
  ];
  if (region) parts.push(`region ${region}`);
  if (b.buyerType)
    parts.push(buyerTypeLabel[b.buyerType as BuyerTypeId] ?? b.buyerType);
  if (l.lead.kind === "sample_kit") {
    parts.push(`incoterm ${l.lead.delivery.incoterm}`);
    if (l.lead.delivery.destination)
      parts.push(`to ${l.lead.delivery.destination}`);
    if (l.lead.delivery.timeline)
      parts.push(`when ${l.lead.delivery.timeline}`);
...
export async function writeToCrm(l: StoredLead): Promise<void> {
  ...
  if (!/Imported\./.test(stdout)) {
    throw new Error(
      `import_leads.py did not confirm import: ${stdout.slice(-300)} ${stderr.slice(-300)}`,
    );
  }
